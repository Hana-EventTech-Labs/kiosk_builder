...
class ProcessingTab(BaseTab):
    ...
    def init_ui(self):
        ...
        # 실제 저장된 배경화면 파일명 로드
        saved_bg = FileHandler.get_background_display_name("process")
    ...
    def _browse_and_update_background(self):
        """배경화면 파일 선택 및 업데이트"""
        FileHandler.browse_background_file(self, self.process_fields["background"], "process")
        saved_bg = FileHandler.get_background_display_name("process")
    ...
    def _reset_background(self):
        ...
        if reply == QMessageBox.Yes:
            FileHandler.delete_background("process")
